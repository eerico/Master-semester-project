...
void FloorPlanAndProfileExtractor::extractAllPlan(OMMesh* inputMesh)
{
    ...
    for (v_it=inputMesh->vertices_begin(); v_it!=v_end; ++v_it) {
        ...
        if ((std::abs(planNumberTmp - planNumber) < 0.001f) && (planNumber >= floorPlanIndex)) {
            ...
            if (std::abs(planNumber - floorPlanIndex) < 0.001f) {
                ...
                for (vv_it=inputMesh->vv_iter(v_it); vv_it; ++vv_it) {
                    ...
                    if (std::abs(planNumberTmp - planNumber) < 0.001f ) {
                        if (std::abs(planNumber - floorPlanIndex) < 0.001f) {
...
{
    if ((std::abs(toX1 - fromX1) < 0.001f  && std::abs(toY1 - fromY1) < 0.001f) || (std::abs(toX2 - fromX2) < 0.001f  && std::abs(toY2 - fromY2) < 0.001f)) {
        return true;
    }
    ...
    float dotProduct = Utils::dotProduct(v1x, v1y, v2x, v2y);

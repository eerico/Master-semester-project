...
SimplificationScene::SimplificationScene(MeshManager *meshManager, std::vector< Curve* >* curveArray)
    : QGraphicsScene(), meshManager(meshManager), firstVertexSelected(0), curveArray(curveArray)
{
...
void SimplificationScene::revertColor() {
    unsigned int size = meshManager->getFloorPlanSize();
    Vertex* iterator = meshManager->getFloorPlan();
...
void SimplificationScene::loadFloorPlan() {
    unsigned int size = meshManager->getFloorPlanSize();
    Vertex* iterator = meshManager->getFloorPlan();
    Edge* currentEdge = iterator->getEdge2();
    for(unsigned int i(0); i < size; ++i) {
...
void SimplificationScene::mousePressEvent(QGraphicsSceneMouseEvent* mouseEvent) {
        ...
        for (unsigned int i(0); i < floorPlanSize; ++i) {
            ...
            if (ellipse->isUnderMouse() && iterator->isValid()){
                ...
                if(currentCurve->begin == 0) {
                    ...
                    if(firstVertexSelected != 0) {
                        iterator->invalid();
                    } else {
                        firstVertexSelected = iterator;
                    }
                } else {
...
void SimplificationScene::updateSceneEllipseAndLineColor() {
